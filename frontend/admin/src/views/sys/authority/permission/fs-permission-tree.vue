    ...
    <template #title="scope">
      <div class="node-title-pane">
        ...
        <div v-if="editable === true" class="node-suffix">
          ...
          <fs-icon v-if="actions.edit !== false && scope.id !== 0" :icon="$fsui.icons.edit" @click.stop="edit(scope)" />
          <fs-icon v-if="actions.remove !== false && scope.id !== 0" :icon="$fsui.icons.remove" @click.stop="remove(scope)" />
        </div>
      </div>
    </template>
  </a-tree>
