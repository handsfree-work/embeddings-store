<template>
  <fs-page>
    ...
    <fs-crud ref="crudRef" v-bind="crudBinding">
      <a-button v-permission="'authority:permission:add'" style="margin-left: 20px" @click="addHandle({})">
...
export default defineComponent({
  ...
  setup() {
    ...
    const { crudBinding, crudRef, crudExpose } = useFs({ createCrudOptions, context: { permission: "authority:permission" } });
    ...
    async function addHandle(item: any) {
      await crudExpose.openAdd({ row: { parent_id: item?.id ?? 0 } });
    }
    ...
    const permission = ref({
      add: hasPermissions("authority:permission:add"),
      edit: hasPermissions("authority:permission:update"),
      remove: hasPermissions("authority:permission:delete")
    });
