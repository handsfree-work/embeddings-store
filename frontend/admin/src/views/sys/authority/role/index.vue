<template>
  <fs-page>
    ...
    <a-modal v-model:visible="authzDialogVisible" width="860px" title="分配权限" @ok="updatePermission">
      <fs-permission-tree ref="permissionTreeRef" v-model:checkedKeys="checkedKeys" :tree="permissionTreeData" :editable="false" checkable :fieldNames="{ key: 'id', label: 'title' }"> </fs-permission-tree>
    </a-modal>
  </fs-page>
</template>
...
function useAuthz() {
  ...
  async function authzOpen(roleId: any) {
    let res = await permissionApi.GetTree();
    permissionTreeData.value = res.list;
    checkedKeys.value = [];
  ...
  async function updateChecked(roleId: any) {
    let res = await api.getPermissionIds(roleId);
    let checkedIds = res.list;
    // 找出所有的叶子节点
